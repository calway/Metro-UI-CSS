...
((Metro, $) => {
	...
	Metro.Component("table", {
		...
		_create: function () {
			...
			if (!element.id()) {
				console.warn(
					"To use all table component features, please set an ID for the table element!",
				);
				element.id(id);
			}
			...
			if (o.viewSaveMode.toLowerCase() === "client") {
				...
				fetch(viewPath)
					...
					.then((view) => {
						if (
							Metro.utils.isValue(view) &&
							Metro.utils.objectLength(view) ===
								Metro.utils.objectLength(this.view)
						) {
							this.view = view;
							this._fireEvent("view-get", {
								source: "server",
					...
					.catch((error) => {
						this._final();
						console.warn(
							`Warning! Error loading view for table ${element.attr("id")}: ${error}`,
						);
					});
			}
		},
		...
		_createInspector: function () {
			...
			$(
				`<div class='table-inspector-header'>${o.inspectorTitle || strings.label_inspector}</div>`,
			).appendTo(inspector);
			...
			const actions = $("<div class='table-inspector-actions'>").appendTo(
				inspector,
			);
		...
		_createHeadsFromHTML: function () {
			...
			if (head.length > 0) {
				$.each(head.find("tr > *"), function () {
					...
					if (item.hasClass("rownum-cell") || item.hasClass("check-cell"))
						return;
					...
					const head_item = {
						...
						sortable:
							...
							(Metro.utils.isValue(item.data("sortable")) &&
								JSON.parse(item.data("sortable")) === true),
						sortDir: dir,
						format: Metro.utils.isValue(item.data("format"))
							? item.data("format")
							: "string",
						formatMask: Metro.utils.isValue(item.data("format-mask"))
							? item.data("format-mask")
							: null,
						clsColumn: Metro.utils.isValue(item.data("cls-column"))
							? item.data("cls-column")
							: "",
						cls: item_class,
						colspan: item.attr("colspan"),
						size: Metro.utils.isValue(item.data("size"))
							? item.data("size")
							: "",
						show: !(
							item.hasClass("hidden") ||
							(Metro.utils.isValue(item.data("show")) &&
								JSON.parse(item.data("show")) === false)
						),

						required: Metro.utils.isValue(item.data("required"))
							? JSON.parse(item.data("required")) === true
							: false,
						field: Metro.utils.isValue(item.data("field"))
							? item.data("field")
							: "input",
						fieldType: Metro.utils.isValue(item.data("field-type"))
							? item.data("field-type")
							: "text",
						validator: Metro.utils.isValue(item.data("validator"))
							? item.data("validator")
							: null,

						template: Metro.utils.isValue(item.data("template"))
							? item.data("template")
							: null,
					};
		...
		_createBottomBlock: function () {
			...
			skip = Metro.utils.isValue(this.wrapperSkip)
				? this.wrapperSkip
				: $("<div>").appendTo(bottom_block);
		...
		_createStructure: function () {
			...
			if (need_sort) {
				...
				$(columns.get(this.sort.colIndex + that.service.length)).addClass(
					`sort-${this.sort.dir}`,
				);
				this.sorting();
			}
		...
		_createInspectorEvents: function () {
			...
			inspector
				...
				.on(Metro.events.inputchange, function () {
					const input = $(this);
		...
		_info: function (start, stop, length) {
			...
			const strings = this.strings;
			const component = element.closest(".table-component");
			const info = Metro.utils.isValue(this.wrapperInfo)
				? this.wrapperInfo
				: component.find(".table-info");
			let text;
			let _start = start;
			let _stop = stop;
			let _length = length;
		...
		_filter: function () {
			...
			let items;
			if (
				(Metro.utils.isValue(this.searchString) &&
					this.searchString.length >= o.searchMinLength) ||
						...
						for (i = 0; i < this.filters.length; i++) {
							...
							result =
								o.filtersOperator.toLowerCase() === "and"
									? result &&
										Metro.utils.exec(this.filters[i], [row, this.heads])
									: result ||
										Metro.utils.exec(this.filters[i], [row, this.heads]);
						}
		...
		_draw: function (cb) {
			...
			let items;
			let checkedItems = [];
			const stored_keys = Metro.storage.getItem(
				o.checkStoreKey.replace("$1", element.attr("id")),
			);
			...
			if (items.length > 0) {
				...
				$(this.component)
					.find(".table-service-check-all input")
					.prop("checked", checkedItems.length);
			} else {
				j = 0;
				$.each(view, function () {
					if (this.show) j++;
		...
		_getItemContent: function (row) {
			...
			if (Metro.utils.isValue(result) && Metro.utils.isValue(format)) {
				if (["number", "int", "float", "money"].indexOf(format) !== -1) {
					result = Metro.utils.parseNumber(
						result,
						thousandSeparator,
						decimalSeparator,
					);
				...
				switch (format) {
					case "date":
						result = formatMask
							? Datetime.from(result, formatMask, locale)
							: datetime(result);
						break;
					case "number":
						result = +result;
		...
		updateItem: function (key, field, value) {
			...
			if (Metro.utils.isNull(fieldIndex)) {
				console.warn(
					`Item is undefined for update. Field ${field} not found in data structure`,
				);
				return this;
			}
		...
		deleteItem: function (fieldIndex, value) {
			...
			this.items = Metro.utils.arrayDeleteByMultipleKeys(
				this.items,
				deleteIndexes,
			);
		...
		_rebuild: function (review) {
			...
			if (need_sort) {
				...
				$(sortable_columns.get(that.sort.colIndex)).addClass(
					`sort-${that.sort.dir}`,
				);
				this.sorting();
			}
			...
			if (mode) {
				ins.show(0, () => {
					ins
						.css({
							top:
								($(globalThis).height() - ins.outerHeight(true)) / 2 +
								pageYOffset,
							left:
								($(globalThis).width() - ins.outerWidth(true)) / 2 +
								pageXOffset,
						})
						.data("open", true);
				});
		...
		export: function (to, mode, filename, options) {
			...
			const _mode = Metro.utils.isValue(mode)
				? mode.toLowerCase()
				: "all-filtered";
			const _filename = Metro.utils.isValue(filename)
				? filename
				: `${element.id() || Hooks.useId()}-export.csv`;
			...
			switch (_mode) {
				...
				case "view": {
					items = this._filter();
					start =
						Number.parseInt(o.rows) === -1
							? 0
							: o.rows * (this.currentPage - 1);
					stop =
						Number.parseInt(o.rows) === -1
							? items.length - 1
							: start + o.rows - 1;
