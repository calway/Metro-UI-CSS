	...
	Metro.Component("input", {
		...
		_createStructure: function () {
			...
			const container = element
				.wrap("<div>")
				.addClass(`input ${element[0].className}`);
			let buttons;
			let clearButton;
			let revealButton;
			let searchButton;
		...
		_drawAutocompleteList: function (val) {
			...
			const items = this.autocomplete.filter(
		...
		setHistoryIndex: function (val) {
			this.historyIndex =
				val >= this.history.length ? this.history.length - 1 : val;
		},
		...
		setHistory: function (history, append) {
			...
			const _history =
				typeof history === "string"
					? history.toArray(o.historyDivider)
					: history;
