((Metro, $) => {
	...
	Metro.doubleSliderSetup = (options) => {
		DoubleSliderDefaultConfig = $.extend(
			{},
			DoubleSliderDefaultConfig,
			options,
		);
	};
	...
	Metro.Component("double-slider", {
		...
		_createSlider: function () {
			...
			const markerMin = $("<button>")
				.attr("type", "button")
				.addClass("marker marker-min")
				.addClass(o.clsMarker)
				.addClass(o.clsMarkerMin);
			const markerMax = $("<button>")
				.attr("type", "button")
				.addClass("marker marker-max")
				.addClass(o.clsMarker)
				.addClass(o.clsMarkerMax);
			const hintMin = $("<div>")
				.addClass("hint hint-min")
				.addClass(`${o.hintPositionMin}-side`)
				.addClass(o.clsHint)
				.addClass(o.clsHintMin);
			const hintMax = $("<div>")
				.addClass("hint hint-max")
				.addClass(`${o.hintPositionMax}-side`)
				.addClass(o.clsHint)
				.addClass(o.clsHintMax);
			let i;
			...
			if (o.showMinMax === true) {
				const min_max_wrapper = $("<div>")
					.addClass("slider-min-max")
					.addClass(o.clsMinMax);
				$("<span>")
					.addClass("slider-text-min")
					.addClass(o.clsMin)
					.html(`${o.min}`)
					.appendTo(min_max_wrapper);
				$("<span>")
			...
			if (o.label) {
				this._addLabel(o.label, slider_wrapper);
			}
		...
		_move: function (e) {
			...
			this[isMin ? "valueMin" : "valueMax"] = this._correct(
				this._convert(cPix, "pix2val"),
			);
		...
		_hint: function () {
			...
			hint.each(function () {
				...
				_hint.text(
					_mask.replace(
						"$1",
						Metro.utils.exec(o.onDrawHint, [
							value.toFixed(Metro.utils.decCount(o.accuracy)),
						]),
					),
				);
			});
		...
		_marker: function () {
			...
			const marker_size = Number.parseInt(
				Metro.utils.getStyleOne(markerMin, "width"),
			);
			...
			if (slider_visible) {
				...
				markerMin.css({
					...
					"margin-top":
						this._convert(this.valueMin, "val2prc") === 0
							? 0
							: (-1 * marker_size) / 2,
				});
				markerMax.css({
					...
					"margin-top":
						this._convert(this.valueMax, "val2prc") === 0
							? 0
							: (-1 * marker_size) / 2,
				});
			}
			...
			complete.css({
				...
				width:
					this._convert(this.valueMax, "val2pix") -
					this._convert(this.valueMin, "val2pix"),
			});
		},
