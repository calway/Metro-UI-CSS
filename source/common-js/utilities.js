((Metro, $) => {
	...
	Metro.utils = {
		...
		elementId: (prefix) =>
			`${prefix}-${new Date().getTime()}${$.random(1, 1000)}`,
		...
		isUrl: (val) =>
			/^(\.\/|\.\.\/|ftp|http|https):\/\/(\w+:?\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@\-\/]))?/.test(
				val,
			),
		...
		isJQuery: (el) =>
			typeof globalThis.jQuery !== "undefined" &&
			el instanceof globalThis.jQuery,
		...
		isRightMouse: (e) =>
			"which" in e ? e.which === 3 : "button" in e ? e.button === 2 : undefined,
		...
		parseMoney: (val) =>
			Number(Number.parseFloat(val.replace(/[^0-9-.]/g, ""))),
