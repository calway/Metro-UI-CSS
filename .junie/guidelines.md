...
Всі компоненти, які ініціалізуться за допомогою `data-role` атрибута, також можуть бути ініціалізовани за домогою функції `Metro.makePlugin()`,
...
Відповідний CSS класс компонента буде автоматично додано до елемента, на якому ініціалізується компонент.

Доступ до API компоненту можна отримати за допомогою:
- функції `Metro.getPlugin()`, яка приймає селектор елемента, та назву компонента і повертає об'єкт компонента.
- функції `$(selector).data('component-name')`, яка повертає об'єкт компонента.

Стилі компонента зазначаються в LESS файлі компонента, який має назву `[component-name].less`.
Всі стилі, які реалізують компоненти, мають світлу та темну версії. Кольори компонента зазначаютсья в відповідних CSS змінних.
Для світлої теми вони зазначаються в селекторі `:root`, а для темної теми в селекторі класса `.dark-side`.

Опції компонента задаються:
- за допомогою атрибутів HTML елемента, на якому ініціалізується компонент в dashed-name форматі і починаються з `data-` (наприклад: `data-show-marker="true"`)
- за допомогою JavaScript об'єкта, який передається в функцію `Metro.makePlugin()`, в такому випадку опції задаються в camelCase форматі.
...
#### 5.4. **API Methods**
   ...
   or
   ```javascript
   const comp = $('#element').data('component-name');
   comp.methodName();
   ```
